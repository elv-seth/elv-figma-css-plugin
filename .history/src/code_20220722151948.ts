...
class Color{
  ...
  b: number;
  constructor(r,g,b) {
    this.r = Math.round(100*255*r);
    this.g = Math.round(100*255*g);
    this.b = Math.round(100*255*b);
  }
}
...
class ColorFill{
  ...
  constructor(fill) {
    if(fill.type === "SOLID") {
      ...
      alert("only SOLID fill types supported at the moment");
    }
  }
}
...
figma.ui.onmessage = (msg) => {
  ...
  if (msg.type === "stylesheet") {
    console.log("stylesheet message received in plugin code")
    figma.ui.close();
    figma.showUI(__html__, { themeColors: true, height: 517, width: 1000});
    console.log(figma);
  ...
  if (msg.type === "export") {
    console.log("export message received in plugin code")
  }
