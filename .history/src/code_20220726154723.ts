...
class ColorFill{
  ...
  constructor(fill) {
    if(fill.type === "SOLID") {
      ...
      this.rgb1 = new Color(fill.color);
...
class LinearGradient {
  ...
  constructor(stops,transform) {
    ...
    for(let i = 0; i < stops.length; i++) {
      this.colors[i] = new Color(stops[i].color);
    }
  }
}
...
function convertToCssString(color: ColorFill) {
  if(color.isGradient) {
    return color.gradient.direction;
  } else {
  return `rgba(${color.rgb1.r},${color.rgb1.g},${color.rgb1.b},${color.rgb1.opacity})`;
  }
}
